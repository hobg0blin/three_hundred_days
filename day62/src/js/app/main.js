...
setup() {
  ...
  //
        ...
        this.videoTex.repeat.set(1, 8, 1)
 ...
 //       this.controls.update()
    ...
    addObjects() {
      ...
      for (let i = 0; i < numStreams; i++) {
        ...
        let posZ = Math.random() * 80
        let streamCube = new THREE.Mesh(new THREE.SphereGeometry(20, 10, 10), new THREE.MeshPhongMaterial({map: this.videoTex}))
        streamCube.position.set(posX, posY, posZ)
   ...
   //   console.log('foo')
       ...
       let elapsedTime = clock.getElapsedTime()
        if (this.streamArray != undefined && this.streamArray != null) {
          for (let stream of this.streamArray) {
            morph(stream.geometry, 10)
          }
    }
