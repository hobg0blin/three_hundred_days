...
//        this.scene.fog = createFog(0xfcba03, 20, 200)
  ...
  this.scene.add(ground)
    ...
    addObjects() {
      ...
      this.modZ = 0
      this.animNum = 0
      const localPlane = new THREE.Plane( new THREE.Vector3( 0, - 1, 0 ), 7 );
      ...
      this.bodyMat = new THREE.MeshPhongMaterial({ color: 'red', clippingPlanes: [localPlane], clipShadows: true})
      let sphereGeo = new THREE.SphereGeometry(7, 128, 128)
      this.spheres =  []
      for (let i =0; i < 4; i ++) {
      let sphere = new THREE.Mesh(sphereGeo, this.bodyMat)
      if (i % 3 == 0) {
      sphere.position.x = -14 + 10*i
      } else {
      sphere.position.z = -24 + 10*i
      }
...
//            console.log('camera pos: ', this.camera.position)
          ...
          if (this.localPlane.constant > 7) {
            inc = false
            this.localPlane.constant = 6.9
          }
          if (this.localPlane.constant < 0) {
            inc = true
          }
          inc ? this.localPlane.constant+=0.1 : this.localPlane.constant-=0.1;
        }
          for (let sphere of this.spheres) {
          morph(sphere.geometry, 128)
          }
          }
