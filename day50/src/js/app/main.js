...
export default class Texture {
    constructor() {
      ...
      function addCrab(model, mat) {
        console.log('model loaded: ', model)
        console.log('mat: ', mat)
        for (let i = 0; i < 50; i++
        )  {
          this.crabs.push(new THREE.Mesh(model, mat))

        }//        this.crab.position.set(0, 45, 0)
//        this.crab.scale.set(2, 2, 2)
 //       this.scene.add(this.crab)
      }
      this.addCrab = addCrab.bind(this)
        // set up scene
      Ammo().then(AmmoLib => {
        Ammo = AmmoLib
        this.setup()
        importSTLModel('models/Crab_t.stl', new THREE.MeshPhongMaterial({color: 'red'}), this.addCrab)
        this.initPhysics()
        this.createObjects()
...
//          if (Math.round(clock.getElapsedTime() % 4 == 0)) {
//            console.log('foo')
//            updateVertices()
 //         }

            this.updatePhysics(clock.getDelta(), clock.getElapsedTime())
            this.controls.update()
