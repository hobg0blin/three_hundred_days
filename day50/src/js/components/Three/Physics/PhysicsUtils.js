...
groundShape = createTerrainShape();
		...
		const groundMaterial = new MeshLambertMaterial( { color: 0xC7C7C7 } );
			...
			function createObjects() {
				...
				let sphereGeometry = ballObject = new Mesh(new SphereGeometry( sphereRadius, 40, 25 ), new MeshPhongMaterial({color: 'red'}));
...
//						clickRequest = true;
			...
			function updatePhysics( deltaTime, elapsedTime ) {
				if (crab == null && this.crabs.length > 0) {
				crab = this.crabs
				let crabMass = 1
				this.crabs.map(crab => {
				crab.scale.set(0.04, 0.04, 0.04)
					quat.set(0, 0, 0, 1)
					pos.set(Math.random() * 10, Math.random() * 550, Math.random() * 10)
					crab.castShadow = true
// failed attempt at actually mapping mesh -- too many faces
//				let triangleMesh = new Ammo.btTriangleMesh
//					console.log('crab faces: ', crabFaces)
//					for (let i = 0; i < crabFaces.array.length-3; i++) {
//					triangleMesh.addTriangle(new Vector3(crabFaces.getX(i), crabFaces.getY(i), crabFaces.getZ(i)), true)
//						console.log('triangle mesh: ', triangleMesh)
//					}
//					console.log('added faces')
					let box = new Box3()
					box.setFromObject(crab)
					console.log(box.min, box.max, box.getSize)

					let crabShape = new Ammo.btSphereShape(2)
					console.log('made mesh')
					let crabBody = createRigidBody(crab, crabShape, crabMass, pos, quat)
					crabBody.setFriction = 1
					crabBody.setRollingFriction = 4
					crabBody.setRestitution = 8
				crab.userData.physicsBody = crabBody;
					rigidBodies.push( crab );
//					console.log('crab: ', crab)
				scene.add( crab );
				physicsWorld.addRigidBody( crabBody );
})
				}
				moveBall()
				let newTime = Math.round((elapsedTime*100))
//				if (newTime % 2 == 0 && newTime != oldTime) {
