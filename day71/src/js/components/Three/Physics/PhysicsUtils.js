...
//			geometry.rotateY( - Math.PI/2)
		...
		function createObjects() {
				...
				const groundMaterial = new MeshLambertMaterial( { transparent: true, opacity: 0.0 } );
...
//					groundMaterial.needsUpdate = true;
			...
			let ground = createParalellepipedWithPhysics(100, 1, 100, 0, pos, quat, groundMaterial)
			quat.set( 0, 0, 0, 1 );
		//crab
			let crabMat = new MeshPhongMaterial({color: 'white', emissive: 0x444444})
			importSTLModel('models/LowPolySkull.stl', crabMat, addBody)
		}
			function addBody(geometry, mat) {
				geometry.scale(0.2, 0.2, 0.2)
				geometry.computeVertexNormals()
				geometry.attributes.position.needsUpdate = true
				let obj = new Mesh(geometry, mat)
