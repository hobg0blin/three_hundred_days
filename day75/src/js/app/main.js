...
//        this.scene.fog = createFog(0xfcba03, 20, 200)
  ...
  this.controls = createControls(this.camera, this.renderer, {center: {position: { x: 0, y: 0, z: 0}}, autorotate: true})
    ...
    addObjects() {
      ...
      let noseBridge = new THREE.Mesh(new THREE.CylinderGeometry(3, 2, 8, 12), new THREE.MeshPhongMaterial({color: 'hotpink'}))
      noseBridge.position.z = -15
      noseBridge.rotation.x = THREE.MathUtils.degToRad(65)
      this.scene.add(noseBridge)

      const radius = 3;  // ui: radius
      const widthSegments = 12;  // ui: widthSegments
      const heightSegments = 8;  // ui: heightSegments
      const phiStart = Math.PI ;  // ui: phiStart
      const phiLength = Math.PI * 2;  // ui: phiLength
      const thetaStart = Math.PI * 0.25;  // ui: thetaStart
      const thetaLength = Math.PI * 0.5;  // ui: thetaLength
      const noseBallGeo = new THREE.SphereGeometry(
          radius,
          widthSegments, heightSegments,
          phiStart, phiLength,
          thetaStart, thetaLength);
      let noseBallL = new THREE.Mesh(noseBallGeo, new THREE.MeshPhongMaterial({ color: 'hotpink'}))
      noseBallL.position.set(-5, 0, -12)
      noseBallL.rotation.x = THREE.MathUtils.degToRad(65)
      let noseBallR = new THREE.Mesh(noseBallGeo, new THREE.MeshPhongMaterial({ color: 'hotpink'}))
      noseBallR.position.set(5, 0, -12)
      noseBallR.rotation.x = THREE.MathUtils.degToRad(65)
      this.scene.add(noseBallL, noseBallR)
      this.teeth = []
...
//              console.log('model: ', this.model)
      ...
      for (let i = -10; i < 10; i++) {
        let newTooth = this.teeth[i + 10]

        if (i % 10 == 0) {
          this.posNeg = Math.random() >= 0.5 ? -1 : 1
        }
        if (i % 2 == 0) {
          newTooth.scale.set(1, 1 + Math.random()*this.posNeg*0.5, newTooth.scale.z - Math.random()*this.posNeg*0.01)
        } else {
          newTooth.scale.set(1, 1 - Math.random()*this.posNeg*0.5, newTooth.scale.z - Math.random()*this.posNeg*0.01)
        }
      }
