...
//this.scene.add(road)
    this.controls = createControls(this.camera, this.renderer)
      ...
      this.skier = []
      this.model = []
      this.addSkier = this.addSkier.bind(this)
      this.addSkier()
        //if I need mouse pos
      ...
      let jumpMat = new THREE.MeshPhongMaterial({color: 'red', wireframe: true})
      let jumpMesh = new THREE.Mesh(jump, jumpMat)
...
//            console.log('camera pos: ', this.camera.position)
  ...
  let toSplice = []
for (let [index, obj] of this.skier.entries()) {
    obj.position.z += 0.75 + inc
    if (obj.position.z < 85) {
      obj.position.y -= (1.1)
      inc += 0.001
      if (this.model.includes(obj)) {
      obj.rotation.x = THREE.MathUtils.degToRad(45)
  } else {
...
addBody(gltf) {
  ...
  let model
  if (this.model.length > 0) {
    console.log('folddsao')
    model = this.model[0].clone()
  } else {
    model = gltf.scene.children[0]
    this.model.push(model.clone())
  }
...
//     this.mixer = new THREE.AnimationMixer(model)
 ...
 //    this.anim = this.mixer.clipAction(gltf.animations[getRandomInt(0, 6)])
