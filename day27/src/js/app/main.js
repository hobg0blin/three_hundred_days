...
import {createFloor, animateFloor} from '../components/Three/ProceduralFloor/Floor.js'
...
//      this.scene.add(helper)
        this.scene.add(this.light[0], new THREE.AmbientLight(color, 2.7))
...
//        this.scene.fog = createFog(0xffea4d, 50, 650)
        ...
        //GROUND
        let terrainMesh = createFloor({width: 540, height: 500, segments: 255, range: 1})

        let groundMesh = createFloor({width: 550, height: 500, segments: 15, range: 0})
        this.floor = terrainMesh
...
//            this.controls.update()
            animateFloor(this.floor.geometry, range)
            if (range < 200) {
                range++
            }
