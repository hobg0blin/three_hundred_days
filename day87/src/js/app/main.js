...
//        this.scene.fog = createFog(0xfcba03, 1, 1000)
  ...
  this.controls = createControls(this.camera, this.renderer)
    ...
    addObjects() {
      ...
      this.particleSystem = createParticles(10000, new THREE.Color(randomColor({format: 'rgb'})), 0.5, {x: 50, y: 50, z: 50}, {x: -50, y: -50, z: -50})
				this.object = new THREE.Object3D();
				this.scene.add( this.object );
        this.object.add(this.particleSystem.mesh)
            // test reflector
        let test =  createBall(1, 8, 'red')
      let floor = createFloor({width: 100, height: 100, segments: 100, range: 10})
        test.position.z = 4
//        this.object.add(test)
        this.scene.add(floor)
      let treeMat = new THREE.MeshPhongMaterial({color: 'black'})
        let tree = deadTree(1, treeMat, 4)
        ...
        this.effect1 = new FilmPass(20, 1, 10, 0)
