...
//				groundBody.setDamping(0.8, 0)
		...
		for (let i = 0; i < 10; i++) {
			console.log('sphere time')
				const sphereGeometry = new Mesh(new SphereGeometry( sphereRadius, 40, 25 ), new MeshPhongMaterial({color: 'red'}));
//				sphereGeometry.translate( 0, 25, 0 );

const sphereShape = new Ammo.btSphereShape(sphereRadius)


				pos.set(Math.random() *  25 * posNeg, 45, Math.random() * 25 * posNeg)
