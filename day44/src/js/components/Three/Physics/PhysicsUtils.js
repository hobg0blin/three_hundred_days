import {MathUtils, ConeGeometry, BoxGeometry, PlaneGeometry, BufferGeometry, SphereGeometry, Mesh, Vector2, Vector3, MeshPhongMaterial, RepeatWrapping, Quaternion, TextureLoader} from 'three'
...
const groundShape = createTerrainShape();
			...
			function generateHeight( width, depth, minHeight, maxHeight ) {
				...
				for ( let j = 0; j < depth; j ++ ) {
					...
					for ( let i = 0; i < width; i ++ ) {
						...
						const height = ( Math.cos( radius * phaseMult ) + 1 ) * 0.5 * hRange + minHeight;
		...
		const groundMaterial = new MeshPhongMaterial( { color: 0xC7C7C7 } );
			...
			function createObjects() {
				...
				sphereGeometry.translate( -25, 25, 0 );
...
//						clickRequest = true;
			...
			function updatePhysics( deltaTime, elapsedTime ) {
				...
				terrainMaxHeight += 0.01*posNeg
				terrainMinHeight -= 0.1*posNeg
				heightData = generateHeight( terrainWidth, terrainDepth, terrainMinHeight, terrainMaxHeight );
				updateVertices()
