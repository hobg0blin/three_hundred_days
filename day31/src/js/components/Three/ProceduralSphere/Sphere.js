...
if (newY < lowestY) {
    ...
    let tex = loader.load('textures/hank.jpg')
    tex.repeat.set(1.8, 1.8, 1.8)
    tex.offset.set(0.0, -0.3)
    let floorMaterial = new MeshPhongMaterial({ map: tex, emissive:0x000000, shininess: 200,/* wireframe: true* for multicolor:  vertexColors: true*/})
...
//} else {
    ...
    let ratio =
 i/floor.attributes.position.array.length
...
floor.attributes.position.setY(i,Z1*rando/min + ratio/1000)

floor.attributes.position.setX(i,X1*(rando/min  + ratio/100))
