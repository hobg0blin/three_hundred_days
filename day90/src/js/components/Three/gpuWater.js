...
import {ShaderChunk, PlaneGeometry, SphereGeometry, MeshPhongMaterial, MeshBasicMaterial, ShaderMaterial, UniformsUtils, ShaderLib, Color, Vector3, Vector2, Mesh, HalfFloatType, WebGLRenderTarget, ClampToEdgeWrapping, NearestFilter, RGBAFormat, UnsignedByteType} from 'three'

let scene, renderer, raycaster, camera
...
function createReactiveWater() {
	console.log('creating water')
				...
				camera = this.camera
				initWater();
				//is there a way to optimize this? make them all a single geometry or something? can't do large numbers right now
				createSpheres();
			...
			function initWater() {
				...
				console.log('height map variable: ', heightmapVariable)
			...
			function createSpheres() {
				...
				const sphereTemplate = new Mesh( new SphereGeometry( 3, 24, 12 ), new MeshPhongMaterial( { color: 0xFFFF00 } ) );
				...
				for ( let i = 0; i < NUM_SPHERES; i ++ ) {
					...
					sphere.userData.velocity = new Vector3(0, -1, 0);
...
//						}
						if ( pos.y <  0.5 ) {
							...
							pos.x = ( Math.random() - 0.5 ) * BOUNDS * 0.7;
							pos.z = ( Math.random() - 0.5 ) * BOUNDS * 0.7;
						}
...
function animateWater() {
  ...
  // Set uniforms: mouse interaction
				...
				for (let sphere of spheres) {
						this.raycaster.set( sphere.position, sphere.userData.velocity );

						const intersects = this.raycaster.intersectObject( meshRay );
...
export { createReactiveWater, animateWater}
