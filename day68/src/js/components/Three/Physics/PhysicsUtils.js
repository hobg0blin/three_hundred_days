...
//				physicsWorld.addRigidBody( groundBody );
		...
		function createObjects() {
			...
			//ship
				importOBJModel('models/SpaceKit/spaceCraft1.obj', 'models/SpaceKit/spaceCraft1.mtl', addBody)
			//hanks
			const stoneMass = 120;
			const stoneHalfExtents = new Vector3( 3.5, 1.5, 64);
			const numStones = 0;
			let hankTex = textureLoader.load('textures/hank.jpg')
			hankTex.repeat.set(1.9, 1.9, 1.9)
			hankTex.offset.set(-0.05, -0.3, 0)
			let stoneMat = new MeshPhongMaterial({ map: hankTex})
			const p = 2
			const q = 6
		...
		quat.set( 0, 0, 0, 1 );
			for ( let i = -4; i < numStones; i +=2 ) {
				for (let j = -numStones; j < 5; j+=2) {
					 ...
					 let object = new Mesh( new SphereGeometry(4, 6, 3), stoneMat );
					let shape = new Ammo.btSphereShape(4)
					pos.set( i * 4, (j * 4), 0 );
					createObject(object, stoneMass, pos, quat, shape);
			}
		}
...
//				obj.position.set(0, 0, 0)
				pos.set(-5, -25, 0)
				quat.set(MathUtils.degToRad(90),0, 0, 1)
...
function updatePhysics( deltaTime ) {
		...
		physicsWorld.stepSimulation( deltaTime, 10 );
			...
			for ( let i = 0, il =this.dispatcher.getNumManifolds(); i < il; i ++ ) {
				...
				let scalingFactor = 1.07
				const fractureImpulse = 250;

				if ( breakable0 && ! collided0 && maxImpulse > fractureImpulse ) {

					const debris = convexBreaker.subdivideByImpact( threeObject0, impactPoint, impactNormal, 0,6 );
				...
				if ( breakable1 && ! collided1 && maxImpulse > fractureImpulse ) {
					...
					const debris = convexBreaker.subdivideByImpact( threeObject1, impactPoint, impactNormal, 0, 6 );
...
function shootBall() {
			...
			const ball = new Mesh( new SphereGeometry( ballRadius, 18, 12 ), ballMaterial );
			...
			ballBody.setLinearVelocity( new Ammo.btVector3( 0, 60, 0 ) );
