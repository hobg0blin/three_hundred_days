...
//        this.scene.fog = createFog(0xfcba03, 1, 1000)
  ...
  this.controls = createControls(this.camera, this.renderer, {center: {x: 0, y: 0, z: 0}, autorotate: true})
    ...
    addObjects() {
      ...
      console.log('effect1: ', this.effect1)
        this.composer.addPass(this.effect1)
      let sky = createSky(this.scene, this.renderer)
      this.sky = sky[0]
      this.sun = sky[1]
      this.addGUI()
      this.city = createCity(this.renderer)
      const floor = createFloor({width: 1200, height: 1200, segments: 50, range: 5, color: 'gray'})
    ...
    render(time, i) {
         ...
         this.renderFlocking()
        if (currentSeconds != Math.round(clock.getElapsedTime() * 10) && Math.round(clock.getElapsedTime() * 10) % 2 == 0  && buildingCount < 300) {
          currentSeconds = Math.round(clock.getElapsedTime())
          addBuilding(this.city)
          if (this.effect1.uniforms.damp.value < 0.990) {
            this.effect1.uniforms.damp.value += 0.001
            console.log('damp: ', this.effect1.uniforms.damp)
          }
          buildingCount++
        }
