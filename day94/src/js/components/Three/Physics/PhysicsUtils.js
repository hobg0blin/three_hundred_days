import {MathUtils, BoxGeometry, SphereGeometry, CylinderGeometry, TorusKnotGeometry, Mesh, Vector2, Vector3, MeshPhongMaterial, MeshStandardMaterial, MeshLambertMaterial, RepeatWrapping, Quaternion, TextureLoader, PlaneGeometry, VideoTexture} from 'three'
...
//			geometry.rotateY( - Math.PI/2)
		...
		function createPhysicsObjects() {
			...
			const groundMaterial = new MeshLambertMaterial( { transparent: true, opacity: 0.1, color: 'blue' } );
				textureLoader.load( 'textures/nice.png', function ( texture ) {
...
//					groundMaterial.needsUpdate = true;
					let tumGeo = new CylinderGeometry(1, 1, 1, 12, 2)
					let tumMat = new MeshStandardMaterial({color: 'white'})
					let tumMesh = new Mesh(tumGeo, tumMat)
					let tumShape = createConvexHullPhysicsShape(tumGeo.attributes.position.array)
					for (let i = 0; i < 80; i++) {
//						pos.set(Math.random() * 4, Math.random()* 50, 0)
//						createObject(tumMesh.clone(), 2, pos, quat, tumShape)
					}
